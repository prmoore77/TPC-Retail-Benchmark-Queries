...
Purpose:  This query against TPC-H is intended to perform an adhoc aggregation
          ...
          Note: We use BIT_STRING_AGG to perform the COUNT(DISTINCT) operation - which
                is a more efficient way to perform this operation in DuckDB.
...
WITH starting_dataset AS (
    SELECT orders.o_custkey AS customer_key
         ...
         , DENSE_RANK() OVER (PARTITION BY orders.o_custkey ORDER BY orders.o_orderkey ASC) AS customer_order_key
         --
...
, rollup_aggregations AS (
    SELECT customer_aggregation_dim.ancestor_level_name AS customer_level_name
         ...
         , facts.customer_key
         --
         , BIT_COUNT (BITSTRING_AGG(customer_order_key, 1, 100)) AS count_distinct_orders
...
/*
 Statistics:
 On TPC-H Scale Factor 1 (with NO filters) - results were:
 Starting dataset row count:   6,001,215
 Output row count                110,515
 Tuple Count:                216,043,740
 Explosion factor:                    36
*/
